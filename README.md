...
| **R** | 建造时旋转 90° |
| 右键(按住) | **瞄准**（移速减半 + 摄像机平滑偏移到鼠标指向处）；建造中点右键取消 |
...
按住右键进入瞄准视角，摄像机平滑偏移到鼠标指向位置：

| 参数 | 值 | 说明 |
|------|-----|------|
| `HEIGHT` | 10.0 | 摄像机固定高度 |
| `TILT_ANGLE` | 52° | 俯视角 |
| `AIM_ACCEL` | 18.0 | 瞄准时加速度 |
| `AIM_MAX_SPEED` | 22.0 | 瞄准时最大移动速度 |
| `AIM_INIT_SPEED` | 1.0 | 瞄准初始速度（由慢到快） |
| `AIM_RETURN_SPEED` | 20.0 | 松开右键后归位速度 |

**偏移公式：** 鼠标指向的地面位置与玩家的 XZ 距离 → 归一化后 × `vision_range`（上限 = `vision_range × 2.5` 鼠标距离）。即鼠标离玩家越远，摄像机偏移越大，最大偏移不超过 `vision_range`。
...
| 地图边界 | ±WORLD_HALF | 超出边界 → 海洋落水持续掉血致死 |
...
| 弹跳冲量 | 随机 1.5~3.0 / 2.5~5.0 | 水平/垂直各不相同 |
| 种类 | 24 种 | 从河狸、蜜蜂到老虎等 |
...
5. 新动物随机属性：体型 0.4~0.5、弹跳冲量 1.5~3.0 / 2.5~5.0

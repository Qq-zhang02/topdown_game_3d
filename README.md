...
- **岩石建模优化**：石头由单个球体改为程序化低多边形石块（细分二十面体 + 顶点随机抖动 + 硬棱角平面法线），主石周围环绕 7 颗小石块、主/暗/亮三色明暗分层；生成时随机朝向 + 0.85~1.15 随机缩放，每块石头外观各不相同。
- **树建模优化**：树冠由单个球体改为 4 个扁平椭球团簇（主体 + 前侧深绿 + 后侧中绿 + 顶部亮绿树尖），球体分段调低走低多边形风格；树干加高至 1.6m、锥度加大；生成时随机朝向 + 0.9~1.1 随机缩放，每棵树外观各不相同。
...
- 瞄准偏移改存视野本地坐标系：旋转视野时瞄准偏移随视角刚性公转，屏幕上始终朝鼠标方向（修复旋转后偏移仍按世界方向计算、不跟随旋转的问题）。
...
- 瞄准偏移存于视野本地坐标系：旋转时随视角刚性公转，屏幕上始终朝鼠标方向；小地图保持世界朝向不跟随旋转
